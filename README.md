...
```
evoke seed follower <dns_name_of_follower> > /tmp/follower-seed.tar
```

If you are on the same node as the master container, you can also export the seed with:
```
sudo docker exec <container_id> evoke seed follower <dns_name_of_follower> > /tmp/follower-seed.tar
```
